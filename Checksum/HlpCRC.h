...
public:
	...
	virtual ICRC CloneCRC() const
	{
		CRC HashInstance = CRC(Width, Polynomial, Init, ReflectIn, ReflectOut, XorOut, CheckValue, Names);
		HashInstance.CRCMask = CRCMask;
		HashInstance.CRCHighBitMask = CRCHighBitMask;
		HashInstance.hash = hash;
		HashInstance.IsTableGenerated = IsTableGenerated;
		HashInstance.CRCTable = CRCTable;

		ICRC hash = make_shared<CRC>(HashInstance);
